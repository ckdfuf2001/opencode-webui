...
describe('preSendStripIfMismatch', () => {
  ...
  it('fires strip-all only once per stub (no repeated wipe)', async () => {
    const now = Date.now()
    // 외국 strip 0건 → strip-all 발동. 같은 stub이 남아 있어도 2회째는 스킵.
    stripMock.mockResolvedValue({ partsRemoved: 0, messagesAffected: 0 })
    stripAllMock.mockResolvedValue({ partsRemoved: 9, messagesAffected: 5 })
    mockMessageList([
      userMsg('u1', 'hello', now - 60_000),
  ...
  it('caps strip-all fires per session even for fresh stubs', async () => {
    const now = Date.now()
    stripMock.mockResolvedValue({ partsRemoved: 0, messagesAffected: 0 })
    stripAllMock.mockResolvedValue({ partsRemoved: 9, messagesAffected: 5 })
    // 실패 반복마다 새 stub(e1→e4)이 생겨도 세션당 3회까지만 발동
    for (const stub of ['e1', 'e2', 'e3']) {
      mockMessageList([
        userMsg('u1', 'hello', now - 60_000),
        mismatchErrorMsg(stub, now - 50_000),
      ])
      const res = await preSendStripIfMismatch('http://x', 'ses-1', '/ws', { providerID: 'opencode', modelID: 'm-1.3' })
      expect(res.strippedAllParts).toBe(9)
    }
    ...
    mockMessageList([
      userMsg('u1', 'hello', now - 60_000),
      mismatchErrorMsg('e4', now - 50_000),
    ])
    const fourth = await preSendStripIfMismatch('http://x', 'ses-1', '/ws', { providerID: 'opencode', modelID: 'm-1.3' })
    expect(stripAllMock).not.toHaveBeenCalled()
    expect(fourth.strippedAllParts).toBe(0)
  })
  it('skips strip and sweep while the session is busy', async () => {
    const now = Date.now()
    vi.stubGlobal('fetch', vi.fn(async (url: unknown) => {
      const u = String(url)
