...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_extract(self):
        try:
            ...
            payload = json.loads(self.rfile.read(length) or b"{}")
            source_path = payload.get("path")
            # fallback: chat_uploads 단독 경로 호환
            if source_path and not os.path.isfile(source_path):
                source_path = self._resolve_fallback(source_path)
            if not source_path or not os.path.isfile(source_path):
                self._json(400, {"error": "Invalid path"})
                return
...
# load_env_file이 채우는 키 접두사 (무관한 키 유입 방지, setdefault — 실 env 우선).
_MANAGED_ENV_PREFIXES = ("OPCODE_", "OFFICE_", "TESSDATA_PREFIX", "DOC_CONVERTER_PORT")
...
def _parse_dotenv(path, include_commented=False):
    out = {}
...
def load_env_file():
    """cwd → exe/스크립트 dir 순으로 .env를 찾아 allowlist 키만 보충한다."""
    for directory in (os.getcwd(), _exe_or_script_dir()):
        candidate = os.path.join(directory, ENV_FILE_NAME)
        if not os.path.isfile(candidate):
            continue
        try:
            pairs = _parse_dotenv(candidate)
        except OSError:
            continue
...
def _gen_env_entries(root):
    """(key, value, active) — ROOT만 활성, 나머지는 주석(필요시 해제)."""
    return [
        ("OPCODE_WEBUI_ROOT", root, True),
        ("DOC_CONVERTER_PORT", "8765", False),
        ("TESSDATA_PREFIX", os.path.join(root, "bin", "tesseract", "tessdata"), False),
    ]


def cmd_gen_env(target_dir=None):
    """`doc-converter gen-env [dir]` — dir/.env 생성(없으면), 부족 키만 추가.

    경로 지정이 없으면 실행한 곳(cwd). 기존 파일·값은 건드리지 않는다.
    """
    import datetime
    directory = os.path.abspath(target_dir or os.getcwd())
    os.makedirs(directory, exist_ok=True)
    path = os.path.join(directory, ENV_FILE_NAME)
    existing = set()
    if os.path.isfile(path):
        try:
            # 주석 키(# KEY=)도 존재로 쳐야 재실행 시 중복 추가가 안 된다
            existing = set(_parse_dotenv(path, include_commented=True))
        except OSError:
            existing = set()
    lines = []
    if not os.path.isfile(path) or not existing:
        lines.append(f"# generated by `doc-converter gen-env` on {datetime.datetime.now().isoformat(timespec='seconds')}")
    added = []
    suggested = []
    for key, value, active in _gen_env_entries(directory):
        if key in existing:
            continue
        lines.append(f"{key}={value}" if active else f"# {key}={value}")
        (added if active else suggested).append(key)
    if lines:
        with open(path, "a", encoding="utf-8") as fh:
            fh.write("\n".join(lines) + "\n")
    detail = ", ".join(added) if added else "nothing to add"
    if suggested:
        detail += f" (optional, commented: {', '.join(suggested)})"
    print(f"{path} ({'created' if not existing else 'updated'}: {detail})")
    return path
...
def main():
    import sys
    argv = sys.argv[1:]
    if argv and argv[0] == "gen-env":
        cmd_gen_env(argv[1] if len(argv) > 1 else None)
        return
    if argv and argv[0] in ("-h", "--help", "help"):
        print("usage: doc-converter [gen-env [dir]]  (default: serve HTTP :8765)")
        return
    load_env_file()
    _reap_stale()
