...
/** ?™ê? abort ì§í›„ ?´ë§??ë¯¸ì²˜ë¦??íƒœë¥??˜ì‚´??ë±ƒì?ê°€ ê¹œë¹¡?´ëŠ” ê²ƒì„ ë§‰ëŠ” ê°€?? */
const RECENTLY_ABORTED_MS = 12_000;
const recentlyAborted = new Map<string, number>();

/** ?„ì†¡ ì¤‘ì¸ ?™ê? user ë©”ì‹œì§€. 2s ?´ë§??ìºì‹œë¥???–´?¨ë„ ? ì??œë‹¤. */
const pendingOptimistic = new Map<string, MessageWithParts>();

/** truncate ì§í›„ opencode ë©”ëª¨ë¦¬ê? ??ëª©ë¡???Œë ¤ì¤????ˆì–´ ë·°ë? ? ì??˜ëŠ” ê°€??
 *  ?œê°„???„ë‹Œ "?œê±°??ë©”ì‹œì§€ ID" ê¸°ì??¼ë¡œ ê±¸ëŸ¬ ??ë©”ì‹œì§€??ì¦‰ì‹œ ?µê³¼?œë‹¤. */
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (optimistic) {
        ...
        // ?´ë¼?´ì–¸???¹íˆ ëª¨ë°”?? ?œê³„ê°€ ?œë²„ë³´ë‹¤ ëª?ì´??´ê¸‹?˜ë©´ created >= ë¹„êµë¡œëŠ”
        // ?¤ì œ ? ì? ë©”ì‹œì§€ë¥?ëª?ì°¾ì•„ ?™ê? ì¹´ë“œê°€ ?¨ì•„ duplicated ë¡?ë³´ì???
        // ?œê³„ ?¤ì°¨ 5ì´??ˆìš© + ?œê·¸?ˆì²˜(?ìŠ¤???Œì¼ëª? ?¼ì¹˜ë¡??ì •?œë‹¤.
...
function reconcileOrphanedStreams(
  ...
  let changed = false;
  // idle ?íƒœ?ì„œ ?ŒíŠ¸ ?†ëŠ” ë¯¸ì™„ë£?assistant ë©”ì‹œì§€??? ë ¹ ì¹´ë“œ?´ë?ë¡??œê±°?œë‹¤.
...
export const useSendPrompt = (opcodeUrl: string | null | undefined, directory?: string) => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // ì§„í–‰ ì¤‘ì¸ ?´ë§(2s)???™ê? ë©”ì‹œì§€ë¥???–´??ê¹œë¹¡?´ëŠ” ê²ƒì„ ë°©ì?: in-flight fetch ì·¨ì†Œ
...
function markSessionMessagesCompleted(
  ...
  for (const msg of data) {
    ...
    changed = true
    // ë¹?placeholder(?ŒíŠ¸ ?†ëŠ” ë¯¸ì™„ë£?ì¹´ë“œ)???„ë£Œ ì²˜ë¦¬ ?€???œê±°?œë‹¤.
