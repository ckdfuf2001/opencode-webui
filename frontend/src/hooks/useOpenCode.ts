...
/** ?ï¿½ï¿½? abort ì§í›„ ?ï¿½ë§??ë¯¸ì²˜ï¿??ï¿½íƒœï¿??ï¿½ì‚´??ë±ƒï¿½?ê°€ ê¹œë¹¡?ï¿½ëŠ” ê²ƒì„ ë§‰ëŠ” ê°€?? */
const RECENTLY_ABORTED_MS = 12_000;
const recentlyAborted = new Map<string, number>();

/** ?ï¿½ì†¡ ì¤‘ì¸ ?ï¿½ï¿½? user ë©”ì‹œì§€. 2s ?ï¿½ë§??ìºì‹œï¿???ï¿½ï¿½?ï¿½ë„ ?ï¿½ï¿½??ï¿½ë‹¤. */
...
/**
 * SSEë¡????´ì˜ ?ŒíŠ¸/ë©”ì‹œì§€ê°€ ?˜ëŸ¬?¤ì–´?¤ë©´ ?°ë¦¬ ?„ì†¡?€ ?œë²„???‘ìˆ˜??ê²?
 * ?? placeholder ?ì²´???œë²„ ë°˜ì˜??fetchë¡??•ì¸????êµì²´?´ì•¼
 * ?¬ë¼ì¡Œë‹¤ ?˜í??¬ë‹¤ ?˜ëŠ” ê¹œë¹¡?„ì´ ?†ë‹¤ ???¬ê¸°?œëŠ” ë³´ë¥˜ë§??´ì œ?˜ê³ 
 * ì¹´ë“œ êµì²´??ëª©ë¡ fetch??realUserArrived??ë§¡ê¸´??
 */
...
/**
 * cancel ?œì ???¡ì•„???¸ìŠ¤?´ìŠ¤ë§?ì¤‘ë‹¨?œë‹¤. abort POSTê°€ ??²Œ ?ë‚˜
 * onSettledê°€ ?????¬ìš©?ê? ?´ë? ?ˆë¡œ ë³´ë‚¸ ?´ì´ ?ˆìœ¼ë©? ê·??´ì˜
 * AbortController/EventSource???ˆë? ê±´ë“œë¦¬ì? ?ŠëŠ”??(? ê·œ ë©”ì‹œì§€ ?¤í­ ë°©ì?).
 * map???¡ì•„??ê²ƒê³¼ ê°™ì? ?¸ìŠ¤?´ìŠ¤ê°€ ?¨ì•„ ?ˆì„ ?Œë§Œ ?”íŠ¸ë¦¬ë? ì§€?´ë‹¤.
 */
...
/** ê±°ë? ë¬¸ì??join ?†ì´ ê¸¸ì´ë§??©ì‚° ???´ë§ë§ˆë‹¤ MBê¸?join ? ë‹¹ ë°©ì?.
 *  join('').length === ê°?ê¸¸ì´???©ì´ë¯€ë¡?ê¸¸ì´ ë¹„êµ ?ì • ê²°ê³¼???™ì¼?˜ë‹¤. */
...
/** SSEê°€ ?œë²„?ë§Œ ?ˆëŠ” ??ë©”ì‹œì§€ë¥?ê°€ë¦¬í‚¤ë©?ê°€ì§?ì¹´ë“œë¥?ë§Œë“¤ì§€ ?Šê³  ëª©ë¡ refetchë¥?
 *  ?ë‹¹ê²?ì²??´ìš©??ë¹¨ë¦¬ ê°€?¸ì˜¨?? reasoning?€ ?œì‘ ì§€?°ì´ ?¬ë?ë¡?300ms,
 *  ê·??¸ëŠ” ?¸ì…˜??800ms ?°ë¡œ?€ë¡?refetch ??£¼ë¥?ë§‰ëŠ”?? */
...
function fastPullMessages(
  ...
  const now = Date.now();
  // reasoning SSE??ë³„ë„ 300ms ?°ë¡œ?€ ???œì‘ ì§€??ì²´ê°???¬ë?ë¡????ì£¼ ?¹ê¸´??
  if (isReasoning) {
...
/** truncate ì§í›„ opencode ë©”ëª¨ë¦¬ï¿½? ??ëª©ë¡???ï¿½ë ¤ï¿????ï¿½ì–´ ë·°ï¿½? ?ï¿½ï¿½??ï¿½ëŠ” ê°€??
 *  ?ï¿½ê°„???ï¿½ë‹Œ "?ï¿½ê±°??ë©”ì‹œì§€ ID" ê¸°ï¿½??ï¿½ë¡œ ê±¸ëŸ¬ ??ë©”ì‹œì§€??ì¦‰ì‹œ ?ï¿½ê³¼?ï¿½ë‹¤. */
...
export const useMessages = (opcodeUrl: string | null | undefined, sessionID: string | undefined, directory?: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // ë°±ì—”??ìºì‹œë¥?ê²½ìœ ??ê°€?¸ì˜¨?????„ë¡ ???´ë§??opencode ?´ë²¤?¸ë£¨?„ë? ?Œë¦¬ì§€ ?ŠëŠ”??
      ...
      if (cached && result.length > 0 && cached.length > 0) {
        ...
        if (cachedLast.info.id === resultLast.info.id && cachedLast.parts.length > resultLast.parts.length) {
          ...
          // ê¸¸ì´ ?©ì‚°?¼ë¡œë§?ë¹„êµ ??join?€ ê±°ë? ? ë‹¹?´ë¼ ê¸¸ì´ ?ì •?ëŠ” ?°ì? ?ŠëŠ”??
      ...
      if (optimistic) {
        ...
        realUserArrived = result.some((m) => {
          ...
          // ?´ì „ ?´ì˜ user ë©”ì‹œì§€ë¥??´ë²ˆ ?„ì†¡?¼ë¡œ ?¤ì¸?˜ì? ?Šë„ë¡??œë²„ ë°˜ì˜ ?œê°?€
          // optimistic ?ì„± ?œê° ?´í›„?¬ì•¼ ?œë‹¤ (?´ë¡ ?¤ì°¨ 2s ?ˆìš©).
        ...
        if (!realUserArrived) {
          // ?™ê? ?„ì†¡ ?´í›„???ì„±???¤ì œ ?œë²„ ë©”ì‹œì§€ê°€ ?ˆìœ¼ë©??´ì´ ì§„í–‰??ê²?
          // ?œëª…/?œê° ë§¤ì¹­??ë¹—ë‚˜ê°€???ë¦° ë°˜ì˜Â·?Œì¼ë©˜ì…˜ ë³€?•Â·í ì§€??ë°œì†¡)
          // placeholder???´ë¦°????ë¶™ë“¤ê³??ˆìœ¼ë©??êµ¬ ?”ë¥˜?˜ê³ 
          // hasActiveSend?’WORKINGê¹Œì? ê³ ì°©?œë‹¤. strict > ë¡??´ì „ ???¤ì¸ ë°©ì?.
      ...
      if (optimistic && !realUserArrived) {
        // ?œë²„???„ì§ ë°˜ì˜??user ë©”ì‹œì§€ê°€ ?†ìœ¼ë©?sending placeholderë¥?refetch
        // ê²°ê³¼???¤ì‹œ ë¶™ì¸?? ì²?refetch(500ms)?ì„œ ??ì±„íŒ…???¬ë¼ì¡Œë‹¤ ?˜í??˜ëŠ”
        // ê¹œë¹¡???†ì´ "??ì±„íŒ…(sending) ???œë²„ ë°˜ì˜ ??êµì²´"ë¡??ˆì •?˜ê²Œ ? ì??œë‹¤.
        // ?? ??ë¶„ì´ ì§€?˜ë„ ë°˜ì˜?˜ì? ?Šìœ¼ë©?? ì‹¤???„ì†¡?¼ë¡œ ë³´ê³  ì¤‘ë‹¨?œë‹¤.
        // (ê¸°ì¡´ 30s ?•ë¦¬???´ì „ ??ë©”ì‹œì§€???ì•„ ê±´ë„ˆ?????ˆì–´ ?êµ¬ ?”ë¥˜?œë‹¤.
        //  ?”ë¥˜?˜ë©´ hasActiveSend?’WORKINGê¹Œì? ê³„ì† ì¼œì§„??)
        // ?¤ì œ ?œë²„ ë©”ì‹œì§€ê°€ ?˜ì¤‘???¤ë©´ ?•ìƒ ?Œë”?œë‹¤ ??placeholder???„ì‹œ UI??ë¿?
        const optimisticCreated = optimistic.info.time?.created ?? 0;
    ...
    refetchOnWindowFocus: false,
    refetchOnReconnect: false,
    // ë³µê? ??ìºì‹œê°€ ? ì•„ê°€ ?¤í”¼??+ ì²˜ìŒë¶€???¤ì‹œ ë¡œë“œ?˜ëŠ” ì²´ê°??ì¤„ì´??30ë¶?? ì?.
    // ?¸ì…˜ ?„í™˜ ??inactive ì¿¼ë¦¬??SessionDetail?ì„œ ì§ì ‘ ?œê±°?˜ë?ë¡?ë©”ëª¨ë¦??„ìˆ˜ ?†ìŒ.
    gcTime: 30 * 60 * 1000,
    ...
    queryFn: async () => {
      ...
      try {
        ...
        queryClient.setQueryData(["opencode", "messages", opcodeUrl, sessionID, directory], (old: MessageListResponse | undefined) => {
          ...
          // SSEê°€ ?ì„œ ?˜ê? ?ˆìœ¼ë©??´ë§ ê²°ê³¼ê°€ ??–´?°ì? ?Šë„ë¡?ë³´ì¡´ ???´ì „?ëŠ” ?™ì¼ ?ìŠ¤?¸ì¼ ?Œë§Œ ? ì???SSE ì¦ë¶„??? ì•„ê°”ë‹¤
          // ê¸¸ì´ê°€ ?¤ë¥´ë©?join ?†ì´ ?ì • (?¤íŠ¸ë¦¬ë° ì¤?99%????ê²½ë¡œ), ê°™ì„ ?Œë§Œ ?´ìš© ?™ë“± ?•ì¸
          ...
          // SSEê°€ ??ê¸¸ë©´ ?œë²„ ?‘ë‹µ??lagging ????–´?°ì? ?ŠëŠ”??
          if (curTextLen > nextTextLen || curToolLen > nextToolLen) return old
          // ?œë²„ê°€ ??ê¸¸ê±°???„ë£Œ ?íƒœê°€ ë°”ë€Œì—ˆ???Œë§Œ êµì²´
...
function reconcileOrphanedStreams(
  ...
  let changed = false;
  // 0-part ë¯¸ì™„ë£?assistant(ghost)??busy ?™ì•ˆ ë§ˆì?ë§?ê²ƒë§Œ ? ì???LLM ?‘ë‹µ ?ì—­??
  // ë°”ë¡œ ë³´ì—¬ì¤€??(?œë²„???´ì´ ?ê¸´ honest ? í˜¸ ??ê°€ì§?ì¹´ë“œê°€ ?„ë‹ˆ??.
  // idle???˜ë©´ ?œê±°???”ë¥˜ ?¬í???ë§‰ê³ , ìºì‹œ???†ëŠ” ?ŒíŠ¸??SSE ?¬í? ?ì„±?€ merge ?¨ê³„?ì„œ ì°¨ë‹¨?œë‹¤.
  const filtered = messages.filter((msg, idx) => {
    ...
    // busy ?™ì•ˆ ë§ˆì?ë§?ghost??? ì? (LLM ?‘ë‹µ ?ì—­ ì¦‰ì‹œ ?œì‹œ)
  ...
  const updated = filtered.map((msg): MessageWithParts => {
    ...
    const parts = msg.parts.map((part) => {
      if (part.type === "tool" && part.state?.status === "running") {
        // input/command/output?€ ? ì??˜ê³  ?íƒœë§?errorë¡??¤ì§‘?”ë‹¤.
        // ?µì§¸ë¡?ê°ˆì•„?ìœ¼ë©?ì¤‘ë‹¨???´ì˜ ëª…ë ¹???”ë©´?ì„œ ?¬ë¼ì§„ë‹¤.
        return {
          ...part,
...
export const useSendPrompt = (opcodeUrl: string | null | undefined, directory?: string) => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // ??ì±„íŒ… ?†ì— sending ?œì‹œ (LLM ìª½ì? ?¨ê?, ?œë²„ ë°˜ì˜ ??êµì²´)
      ...
      const sseMergePart = (part: MessageWithParts["parts"][number], delta?: string) => {
        ...
        queryClient.setQueryData<MessageListResponse>(key, (old) => {
          ...
          if (idx === -1) {
            // reasoning ?´ë²¤?¸ëŠ” ë³„ë„ ?°ë¡œ?€(300ms)ë¡???ë¹¨ë¦¬ ?¹ê¸´?????œì‘ ì²´ê° ê°œì„ 
      ...
      const sseHandle = (e: MessageEvent) => {
        try {
          ...
          if (t === "message.part.updated") {
            ...
            // ì¦‰ì‹œ Working ë§ˆí¬ ?´ì œ ??2s ?´ë§ ?€ê¸??†ì´ ìºì‹œ?ì„œ ì§ì ‘ ?œê±°
    ...
    onSettled: (_data, _error, variables) => {
      ...
      // sending?€ ?œë²„???•ìƒ ë°˜ì˜?˜ì–´ ?”ë©´??ë¿Œë ¤ì§??Œê¹Œì§€ ? ì? ??useMessages??realUserArrived?ì„œ êµì²´
      // ?¤íŒ¨/?€?„ì•„???€ë¹„í•´ 30ì´??¤ì—ë§?ê°•ì œ ?•ë¦¬ (?ˆë¬´ ?¼ì° ì§€?°ì? ?ŠìŒ)
      setTimeout(() => {
...
export const useAbortSession = (opcodeUrl: string | null | undefined, directory?: string) => {
  ...
  return useMutation({
    ...
    onMutate: async (sessionID) => {
      // ??²Œ ?ë‚˜??abort POST??onSettledê°€ cancel ì§í›„ ?ˆë¡œ ë³´ë‚¸ ?´ì„
      // ì£½ì´ì§€ ?Šë„ë¡? ì§€ê¸?ì§„í–‰ ì¤‘ì¸ ?¸ìŠ¤?´ìŠ¤ë¥??¡ì•„?”ë‹¤ (contextë¡??„ë‹¬).
    ...
    onSettled: (_data, _error, sessionID, context) => {
      ...
      // cancel ?´í›„ ?ˆë¡œ ?œì‘???„ì†¡?€ ê±´ë“œë¦¬ì? ?ŠëŠ”?????¡ì•„??ê²ƒë§Œ ?•ë¦¬
      ...
      // ???´ì´ ?´ë? ?Œê³  ?ˆìœ¼ë©?ë©”ì‹œì§€ ?íƒœ???????Œìœ  ???„ë£Œ ë§ˆí‚¹ ?ëµ
...
function markSessionMessagesCompleted(
  ...
  for (const msg of data) {
    ...
    changed = true
    // ï¿?placeholder(?ï¿½íŠ¸ ?ï¿½ëŠ” ë¯¸ì™„ï¿?ì¹´ë“œ)???ï¿½ë£Œ ì²˜ë¦¬ ?ï¿???ï¿½ê±°?ï¿½ë‹¤.
    ...
    const patchedParts = msg.parts.map((part) => {
      if ((part as { type?: string }).type === 'tool' && (part as { state?: { status?: string } }).state?.status === 'running') {
        // input/command/output ? ì? ???íƒœë§?errorë¡?(?µì§¸ë¡?ê°ˆì•„?ìœ¼ë©?ëª…ë ¹???¬ë¼ì§„ë‹¤)
  ...
  useEffect(() => {
    ...
    const mergePart = (part: MessageWithParts["parts"][number], delta?: string) => {
      ...
      queryClient.setQueryData<MessageListResponse>(key, (old) => {
        ...
        const mid = (part as { messageID: string }).messageID;
        const idx = old.findIndex((m) => m.info.id === mid);
        // ìºì‹œ???†ëŠ” message id???ŒíŠ¸??ì¶”ì • role ë¡???ì¹´ë“œë¥?ë§Œë“¤ì§€ ?Šê³ ,
        // ëª©ë¡ refetchë¥??ë‹¹ê²??œë²„???¤ì œ ë©”ì‹œì§€ë¥?ë¹¨ë¦¬ ê°€?¸ì˜¨??
