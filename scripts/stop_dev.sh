...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  powershell.exe -NoProfile -Command "\$cwd = '$CWD' -replace '/', '\\\\'; Get-CimInstance Win32_Process | Where-Object { \$_.Name -eq 'doc-converter.exe' -and \$_.ExecutablePath -like (\$cwd + '*') } | ForEach-Object { taskkill /PID \$_.ProcessId /T /F 2>\$null | Out-Null }"
fi
# also try generic pkill for opencode in this tree
