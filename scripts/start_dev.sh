...
echo "[DEV START] pid $! logs: logs/dev.log / logs/dev.err.log"
for i in {1..15}; do
  sleep 1
  if curl -sf -m 2 "http://127.0.0.1:5002/api/health" >/dev/null 2>&1 || curl -sf -m 2 "http://127.0.0.1:5001/api/health" >/dev/null 2>&1; then
