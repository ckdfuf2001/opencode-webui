...
# Use stdbuf for line-buffered logs so background console output appears immediately in logs
if command -v stdbuf >/dev/null 2>&1; then
  nohup stdbuf -oL -eL pnpm dev > logs/dev.log 2> logs/dev.err.log &
else
  nohup pnpm dev > logs/dev.log 2> logs/dev.err.log &
fi
PID=$!
echo $PID > logs/dev.pid
# Ensure log file is created and has content
sleep 0.5
echo "[DEV START] pid $PID logs: logs/dev.log / logs/dev.err.log / pid logs/dev.pid (unbuffered)"
for i in {1..30}; do
  sleep 1
