...
# 이 파일이 직접 읽는 env 키 접두사. .env 자동로드가 무관한 키(PORT 등)를
# 프로세스 환경에 흘리지 않게 allowlist로만 채운다 (setdefault — 실 env 우선).
_MANAGED_ENV_PREFIXES = ("OPCODE_", "OFFICE_", "TESSDATA_PREFIX", "DOC_CONVERTER_PORT", "UNSPLASH_ACCESS_KEY")


def _exe_or_script_dir() -> str:
    if getattr(sys, "frozen", False):
        return os.path.dirname(os.path.abspath(sys.executable))
    return os.path.dirname(os.path.abspath(__file__))


def _parse_dotenv(path: str, include_commented: bool = False) -> dict[str, str]:
    out: dict[str, str] = {}
...
def _gen_env_entries(root: str) -> list[tuple[str, str, bool]]:
    """(key, value, active) — ROOT만 활성, 나머지는 주석(필요시 해제)."""
    return [
        ("OPCODE_WEBUI_ROOT", root, True),
        ("OPCODE_WEBUI_BACKEND", "http://127.0.0.1:5002", False),
        ("OPCODE_WEBUI_WORKSPACE", os.path.join(root, "workspace"), False),
        ("OPCODE_WEBUI_REPOS", os.path.join(root, "workspace", "repos"), False),
        ("OFFICE_BRIDGE_HOST", "127.0.0.1", False),
        ("OFFICE_BRIDGE_PORT", "8766", False),
        ("TESSDATA_PREFIX", os.path.join(root, "bin", "tesseract", "tessdata"), False),
    ]


def cmd_gen_env(target_dir: str | None = None) -> str:
    """`office-mcp gen-env [dir]` — dir/.env 생성(없으면), 부족 키만 추가.
    ...
    if not os.path.isfile(path) or not existing:
        lines.append(f"# generated by `office-mcp gen-env` on {datetime.datetime.now().isoformat(timespec='seconds')}")
...
def main() -> int:
    ...
    if argv and argv[0] in ("-h", "--help", "help"):
        print("usage: office-mcp [gen-env [dir]]  (default transport: stdio)")
        return 0
    logging.basicConfig(
